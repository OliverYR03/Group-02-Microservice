...
public class PlatosWebController {
	...
	.orElseThrow(() -> new ResourceNotFoundException("Este plato no se ha podido ubicar : : " + platosId));
	
	
	platos.setNombre(platosDetails.getNombre());
	platos.setDescripcion(platosDetails.getDescripcion());
	platos.setPrecio(platosDetails.getPrecio());
	platos.setTipoplato(platosDetails.getTipoplato());
	platos.setImagen(platosDetails.getImagen());
